...
namespace Hi3Helper.Http
{
    internal static class IO
    {
            ...
            CancellationToken token)
        {
            ...
            double _maximumBytesPerSecond = 0;
            double bitPerUnit = 0;
        ...
        StartWrite:
            ...
            try
            {
                ...
                if (isNetworkStreamFromExternal && networkStream == null)
                    throw new NullReferenceException("networkStream argument cannot be null when isNetworkStreamFromExternal is set!");
            ...
            catch (Exception)
            {
                ...
                await Task.Delay(session.RetryAttemptInterval);
                goto StartWrite;
            }
            finally
            {
            ...
            void CalculateBps()
            {
                ...
                _maximumBytesPerSecond = DownloadClient.DownloadSpeedLimitBase / threadNum;
                bitPerUnit = 940 - (((double)threadNum - 2) / (16 - 2) * 400);
            }
            ...
            async Task ThrottleAsync()
            {
                ...
                if (_maximumBytesPerSecond <= 0 || written <= 0)
                    return;
                ...
                if (elapsedMilliseconds > 0)
                {
                    ...
                    if (bps > _maximumBytesPerSecond)
                    {
                        ...
                        double wakeElapsed = written * bitPerUnit / _maximumBytesPerSecond;
                        double toSleep = wakeElapsed - elapsedMilliseconds;

                        if (toSleep > 1)
                        {
                            // The time to sleep is more then a millisecond, so sleep.
                            await Task.Delay(TimeSpan.FromMilliseconds(toSleep));
        ...
        private static async ValueTask<HttpResponseInputStream> CreateStreamFromSessionAsync(ChunkSession session, CancellationToken token)
        {
